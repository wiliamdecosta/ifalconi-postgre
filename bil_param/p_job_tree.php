...
<?php
	$queryApp = "select code from p_job_type where p_job_type_id=".$_GET["P_JOB_TYPE_ID"];
	//die($queryApp);
	$dbConn->query($queryApp);
	$dbConn->next_record();
	$appCode = $dbConn->f("CODE");
	$query = "SELECT P_JOB_ID P_JOB_ID, NVL (PARENT_ID, 0) PARENT_ID,CODE, PROCEDURE_NAME,DESCRIPTION "
		. "FROM (SELECT   p_job_id, parent_id, CODE , NVL (PROCEDURE_NAME,'0') PROCEDURE_NAME,DESCRIPTION, "
		. "LISTING_NO FROM P_JOB WHERE P_JOB_TYPE_ID = ".$_GET["P_JOB_TYPE_ID"]." "
		. "ORDER BY NVL (PARENT_ID, 0), LISTING_NO) x START WITH x.PARENT_ID IS NULL "
		. "CONNECT BY PRIOR x.p_job_id = x.parent_id ORDER SIBLINGS BY NVL (listing_no, 9999)";
		//die($query);
	$dbConn->query($query);
?>
...
var TREE_ITEMS =
 [
  ['<?=$appCode?>', 'p_job.php?PARENT_ID=0&PARENT_CODE=-&P_JOB_TYPE_ID=<?=$_GET["P_JOB_TYPE_ID"]?>&JOBTYPE_CODE=<?=$_GET["JOBTYPE_CODE"]?>&P_APPLSearchs_keyword=<?=$_GET["P_APPLSearchs_keyword"]?>&P_APPLGridPage=<?=$_GET["P_APPLGridPage"]?>'
...
<?php
	...
	while ($dbConn->next_record()) {
		if ($dbConn->f("P_JOB_ID")!=$bdmnid) {
			$bdmnid=$dbConn->f("P_JOB_ID");
			...
			$nplevel = $dbConn->f("P_JOB_ID");
			$fileName = $dbConn->f("PROCEDURE_NAME");
			echo "['" . $dbConn->f("CODE");
			if ($fileName=="") {
				echo "',''";
			} else {
...
//					.$_GET["P_APPLICATION_ID"]."'";
				echo "','p_job.php?PARENT_ID=" . $dbConn->f("P_JOB_ID") .
				     "&PARENT_CODE=" . $dbConn->f("CODE") . 
				     "&P_JOB_TYPE_ID=" . $_GET["P_JOB_TYPE_ID"] .
				     "&JOBTYPE_CODE=" . $_GET["JOBTYPE_CODE"] .
