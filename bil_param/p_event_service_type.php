<?php
//Include Common Files @1-1BA7524F
define("RelativePath", "..");
...
//Class_Initialize Event @2-DE92E9F1
    function clsGridP_EVENT_SERVICE_TYPE($RelativePath, & $Parent)
    {
        ...
        $this->LAYANAN = & new clsControl(ccsLabel, "LAYANAN", "LAYANAN", ccsText, "", CCGetRequestParam("LAYANAN", ccsGet, NULL), $this);
        $this->EVENT_NAME = & new clsControl(ccsLabel, "EVENT_NAME", "EVENT_NAME", ccsText, "", CCGetRequestParam("EVENT_NAME", ccsGet, NULL), $this);
        $this->RATED_VALID_FROM = & new clsControl(ccsLabel, "RATED_VALID_FROM", "RATED_VALID_FROM", ccsText, "", CCGetRequestParam("RATED_VALID_FROM", ccsGet, NULL), $this);
        $this->RATED_VALID_TO = & new clsControl(ccsLabel, "RATED_VALID_TO", "RATED_VALID_TO", ccsText, "", CCGetRequestParam("RATED_VALID_TO", ccsGet, NULL), $this);
...
//Show Method @2-CD1C4948
    function Show()
    {
        ...
        if (!$this->IsEmpty) {
            $this->ControlsVisible["LAYANAN"] = $this->LAYANAN->Visible;
            ...
            while ($this->ForceIteration || (($this->RowNumber < $this->PageSize) &&  ($this->HasRecord = $this->DataSource->has_next_record()))) {
                ...
                $this->LAYANAN->SetValue($this->DataSource->LAYANAN->GetValue());
...
//GetErrors Method @2-4DF11C32
    function GetErrors()
    {
        $errors = "";
        $errors = ComposeStrings($errors, $this->LAYANAN->Errors->ToString());
...
//DataSource Variables @2-BFB4A6B0
    var $Parent = "";
    var $CCSEvents = "";
    var $CCSEventResult;
    var $ErrorBlock;
...
//DataSourceClass_Initialize Event @2-1AD5BB98
    function clsP_EVENT_SERVICE_TYPEDataSource(& $Parent)
    {
        $this->Parent = & $Parent;
        $this->ErrorBlock = "Grid P_EVENT_SERVICE_TYPE";
        $this->Initialize();
        $this->LAYANAN = new clsField("LAYANAN", ccsText, "");
        
        $this->EVENT_NAME = new clsField("EVENT_NAME", ccsText, "");
        
        $this->RATED_VALID_FROM = new clsField("RATED_VALID_FROM", ccsText, "");
        
        $this->RATED_VALID_TO = new clsField("RATED_VALID_TO", ccsText, "");
...
//Open Method @2-D2D15C4E
    function Open()
    {
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeBuildSelect", $this->Parent);
        $this->CountSQL = "SELECT COUNT(*) FROM (select P_EVENT_SERVICE_TYPE_ID, LAYANAN, EVENT_CODE, EVENT_NAME, \n" .
        ...
        "where P_SERVICE_TYPE_ID = " . $this->SQLValue($this->wp->GetDBValue("2"), ccsFloat) . "\n" .
        "and ( upper(EVENT_CODE) like upper('%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%') or upper(EVENT_NAME) like upper('%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%') )) cnt";
        $this->SQL = "select P_EVENT_SERVICE_TYPE_ID, LAYANAN, EVENT_CODE, EVENT_NAME, \n" .
        "to_char(RATED_VALID_FROM,'DD-MON-YYYY') AS RATED_VALID_FROM,\n" .
        "to_char(RATED_VALID_TO,'DD-MON-YYYY') AS RATED_VALID_TO, \n" .
        "DESCRIPTION, P_SERVICE_TYPE_ID, P_EVENT_TYPE_ID, CREATION_DATE, CREATED_BY, UPDATED_DATE, UPDATED_BY\n" .
        "from V_P_EVENT_SERVICE_TYPE\n" .
        "where P_SERVICE_TYPE_ID = " . $this->SQLValue($this->wp->GetDBValue("2"), ccsFloat) . "\n" .
        "and ( upper(EVENT_CODE) like upper('%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%') or upper(EVENT_NAME) like upper('%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%') )";
...
//SetValues Method @2-E554F008
    function SetValues()
    {
        $this->LAYANAN->SetDBValue($this->f("LAYANAN"));
        $this->EVENT_NAME->SetDBValue($this->f("EVENT_NAME"));
        $this->RATED_VALID_FROM->SetDBValue($this->f("RATED_VALID_FROM"));
        $this->RATED_VALID_TO->SetDBValue($this->f("RATED_VALID_TO"));
...
//Class_Initialize Event @3-C3D99409
...
//Class_Initialize Event @28-01370D90
    function clsRecordp_event_service_type1($RelativePath, & $Parent)
    {
        ...
        if($this->Visible)
        {
            ...
            $this->RATED_VALID_FROM = & new clsControl(ccsTextBox, "RATED_VALID_FROM", "RATED VALID FROM", ccsText, "", CCGetRequestParam("RATED_VALID_FROM", $Method, NULL), $this);
            ...
            $this->RATED_VALID_TO = & new clsControl(ccsTextBox, "RATED_VALID_TO", "RATED VALID TO", ccsText, "", CCGetRequestParam("RATED_VALID_TO", $Method, NULL), $this);
            ...
            $this->EVENT_NAME = & new clsControl(ccsTextBox, "EVENT_NAME", "P EVENT TYPE ID", ccsText, "", CCGetRequestParam("EVENT_NAME", $Method, NULL), $this);
...
//CheckErrors Method @28-828A75F5
    function CheckErrors()
    {
        $errors = false;
...
//DataSource Variables @28-11EC9B2E
    var $Parent = "";
    var $CCSEvents = "";
    var $CCSEventResult;
    var $ErrorBlock;
...
//DataSourceClass_Initialize Event @28-0DC10410
...
//Open Method @28-944C34B9
    function Open()
    {
        ...
        "DESCRIPTION, P_SERVICE_TYPE_ID, P_EVENT_TYPE_ID, CREATION_DATE, CREATED_BY, UPDATED_DATE, UPDATED_BY\n" .
        "FROM V_P_EVENT_SERVICE_TYPE\n" .
        "where P_EVENT_SERVICE_TYPE_ID = " . $this->SQLValue($this->wp->GetDBValue("1"), ccsFloat) . "\n" .
        "";
        $this->Order = "";
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeExecuteSelect", $this->Parent);
        $this->query(CCBuildSQL($this->SQL, $this->Where, $this->Order));
...
//Insert Method @28-52982D1D
    function Insert()
    {
        ...
        $this->cp["RATED_VALID_FROM"] = new clsSQLParameter("ctrlRATED_VALID_FROM", ccsText, "", "", $this->RATED_VALID_FROM->GetValue(true), "", false, $this->ErrorBlock);
        $this->cp["RATED_VALID_TO"] = new clsSQLParameter("ctrlRATED_VALID_TO", ccsText, "", "", $this->RATED_VALID_TO->GetValue(true), "", false, $this->ErrorBlock);
        ...
        $this->SQL = "INSERT INTO P_EVENT_SERVICE_TYPE(\n" .
        ...
        "" . $this->SQLValue($this->cp["P_EVENT_TYPE_ID"]->GetDBValue(), ccsFloat) . ",\n" .
        "to_date('" . $this->SQLValue($this->cp["RATED_VALID_FROM"]->GetDBValue(), ccsText) . "','dd/mm/yyyy'),\n" .
        "to_date('" . $this->SQLValue($this->cp["RATED_VALID_TO"]->GetDBValue(), ccsText) . "','dd/mm/yyyy'),\n" .
...
//Update Method @28-728127D9
    function Update()
    {
        ...
        "RATED_VALID_FROM=to_date('" . $this->SQLValue($this->cp["RATED_VALID_FROM"]->GetDBValue(), ccsText) . "','dd/mm/yyyy'),\n" .
        "RATED_VALID_TO=to_date('" . $this->SQLValue($this->cp["RATED_VALID_TO"]->GetDBValue(), ccsText) . "','dd/mm/yyyy'),\n" .
