...
class clsGridP_DAY_CATEGORY { //P_DAY_CATEGORY class @2-DB1D402E
...
//Class_Initialize Event @2-B65A464F
    function clsGridP_DAY_CATEGORY($RelativePath, & $Parent)
    {
        global $FileName;
        global $CCSLocales;
        global $DefaultDateFormat;
        $this->ComponentName = "P_DAY_CATEGORY";
        $this->Visible = True;
        $this->Parent = & $Parent;
        $this->RelativePath = $RelativePath;
        $this->Errors = new clsErrors();
        $this->ErrorBlock = "Grid P_DAY_CATEGORY";
        $this->Attributes = new clsAttributes($this->ComponentName . ":");
        $this->DataSource = new clsP_DAY_CATEGORYDataSource($this);
        ...
        $this->ADLink->Page = "p_day_category.php";
        ...
        $this->P_DAY_CATEGORY_Insert = & new clsControl(ccsLink, "P_DAY_CATEGORY_Insert", "P_DAY_CATEGORY_Insert", ccsText, "", CCGetRequestParam("P_DAY_CATEGORY_Insert", ccsGet, NULL), $this);
        $this->P_DAY_CATEGORY_Insert->Page = "p_day_category.php";
    }
    ...
    function Show()
    {
        ...
        if (!$this->IsEmpty) {
            ...
            while ($this->ForceIteration || (($this->RowNumber < $this->PageSize) &&  ($this->HasRecord = $this->DataSource->has_next_record()))) {
                ...
                $this->ADLink->Parameters = CCAddParam($this->ADLink->Parameters, "P_DAY_CATEGORY_ID", $this->DataSource->f("P_DAY_CATEGORY_ID"));
        ...
        $this->P_DAY_CATEGORY_Insert->Parameters = CCGetQueryString("QueryString", array("P_DAY_CATEGORY_ID", "ccsForm"));
        $this->P_DAY_CATEGORY_Insert->Parameters = CCAddParam($this->P_DAY_CATEGORY_Insert->Parameters, "FLAG", "ADD");
        $this->Navigator->Show();
        $this->P_DAY_CATEGORY_Insert->Show();
        $Tpl->parse();
        $Tpl->block_path = $ParentPath;
...
} //End P_DAY_CATEGORY Class @2-FCB6E20C

class clsP_DAY_CATEGORYDataSource extends clsDBConn {  //P_DAY_CATEGORYDataSource Class @2-7744288D

//DataSource Variables @2-2EBA9628
    var $Parent = "";
    var $CCSEvents = "";
    var $CCSEventResult;
    var $ErrorBlock;
...
//DataSourceClass_Initialize Event @2-53B741FC
    function clsP_DAY_CATEGORYDataSource(& $Parent)
    {
        $this->Parent = & $Parent;
        $this->ErrorBlock = "Grid P_DAY_CATEGORY";
        $this->Initialize();
...
//Open Method @2-6D9235DF
    function Open()
    {
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeBuildSelect", $this->Parent);
        $this->CountSQL = "SELECT COUNT(*) FROM (select * from p_day_category\n" .
        "where upper(code) like upper('%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%')) cnt";
        $this->SQL = "select * from p_day_category\n" .
...
//SetValues Method @2-8D109636
    function SetValues()
    {
        $this->CODE->SetDBValue($this->f("CODE"));
        $this->DESCRIPTION->SetDBValue($this->f("DESCRIPTION"));
        $this->DLink->SetDBValue($this->f("P_DAY_CATEGORY_ID"));
        $this->ADLink->SetDBValue($this->f("P_DAY_CATEGORY_ID"));
        $this->P_DAY_CATEGORY_ID->SetDBValue(trim($this->f("P_DAY_CATEGORY_ID")));
    }
//End SetValues Method

} //End P_DAY_CATEGORYDataSource Class @2-FCB6E20C

class clsRecordP_DAY_CATEGORYSearch { //P_DAY_CATEGORYSearch Class @3-3CCF505A
...
//Class_Initialize Event @3-0AE65667
...
class clsRecordp_day_category1 { //p_day_category1 Class @19-B04495B9
...
//Class_Initialize Event @19-9DAAFAAC
    function clsRecordp_day_category1($RelativePath, & $Parent)
    {
        ...
        $this->RelativePath = $RelativePath;
        $this->Errors = new clsErrors();
        $this->ErrorBlock = "Record p_day_category1/Error";
        $this->DataSource = new clsp_day_category1DataSource($this);
        $this->ds = & $this->DataSource;
        ...
        if($this->Visible)
        {
            $this->ComponentName = "p_day_category1";
            $this->Attributes = new clsAttributes($this->ComponentName . ":");
            $CCSForm = explode(":", CCGetFromGet("ccsForm", ""), 2);
            if(sizeof($CCSForm) == 1)
                $CCSForm[1] = "";
...
//CheckErrors Method @19-1483B285
    function CheckErrors()
    {
        $errors = false;
...
} //End p_day_category1 Class @19-FCB6E20C

class clsp_day_category1DataSource extends clsDBConn {  //p_day_category1DataSource Class @19-B9A944E6

//DataSource Variables @19-F6A13F85
    var $Parent = "";
    var $CCSEvents = "";
    var $CCSEventResult;
    var $ErrorBlock;
...
//DataSourceClass_Initialize Event @19-D1B083CB
    function clsp_day_category1DataSource(& $Parent)
    {
        $this->Parent = & $Parent;
        $this->ErrorBlock = "Record p_day_category1/Error";
        $this->Initialize();
...
//Insert Method @19-3330F828
    function Insert()
    {
        ...
        $this->SQL = "INSERT INTO P_DAY_CATEGORY(P_DAY_CATEGORY_ID, CODE, DESCRIPTION, CREATION_DATE, CREATED_BY, UPDATED_DATE, UPDATED_BY)\n" .
        "VALUES\n" .
        "(GENERATE_ID('TRB','P_DAY_CATEGORY','P_DAY_CATEGORY_ID'),'" . $this->SQLValue($this->cp["CODE"]->GetDBValue(), ccsText) . "','" . $this->SQLValue($this->cp["DESCRIPTION"]->GetDBValue(), ccsText) . "',sysdate, '" . $this->SQLValue($this->cp["CREATED_BY"]->GetDBValue(), ccsText) . "',sysdate, '" . $this->SQLValue($this->cp["UPDATED_BY"]->GetDBValue(), ccsText) . "')";
...
//Update Method @19-444D221C
    function Update()
    {
        ...
        $this->SQL = "UPDATE P_DAY_CATEGORY SET \n" .
...
} //End p_day_category1DataSource Class @19-FCB6E20C
...
// Controls
$P_DAY_CATEGORY = & new clsGridP_DAY_CATEGORY("", $MainPage);
$P_DAY_CATEGORYSearch = & new clsRecordP_DAY_CATEGORYSearch("", $MainPage);
$p_day_category1 = & new clsRecordp_day_category1("", $MainPage);
$MainPage->P_DAY_CATEGORY = & $P_DAY_CATEGORY;
$MainPage->P_DAY_CATEGORYSearch = & $P_DAY_CATEGORYSearch;
$MainPage->p_day_category1 = & $p_day_category1;
$P_DAY_CATEGORY->Initialize();
$p_day_category1->Initialize();
...
//Execute Components @1-D1137B42
$P_DAY_CATEGORYSearch->Operation();
$p_day_category1->Operation();
//End Execute Components

//Go to destination page @1-187F4D92
if($Redirect)
{
    $CCSEventResult = CCGetEvent($CCSEvents, "BeforeUnload", $MainPage);
    $DBConn->close();
    header("Location: " . $Redirect);
    unset($P_DAY_CATEGORY);
    unset($P_DAY_CATEGORYSearch);
    unset($p_day_category1);
    unset($Tpl);
    exit;
}
//End Go to destination page

//Show Page @1-608E485C
$P_DAY_CATEGORY->Show();
$P_DAY_CATEGORYSearch->Show();
$p_day_category1->Show();
$Tpl->block_path = "";
