...
ORDER BY coalesce(listing_no,999)">
			<Components>
				...
				<Link id="128" visible="Yes" fieldSourceType="DBColumn" dataType="Text" html="True" hrefType="Page" urlType="Relative" preserveParameters="GET" name="P_APPL_Insert" PathID="P_APPLGridP_APPL_Insert" hrefSource="p_application.ccp" wizardUseTemplateBlock="False" removeParameters="P_APPLICATION_ID;FLAG;">
					<Components/>
					<Events>
					</Events>
					<LinkParameters>
		...
		<Record id="37" sourceType="SQL" urlType="Relative" secured="False" allowInsert="True" allowUpdate="True" allowDelete="True" validateData="True" preserveParameters="GET" returnValueType="Number" returnValueTypeForDelete="Number" returnValueTypeForInsert="Number" returnValueTypeForUpdate="Number" connection="Conn" name="P_APPLForm" errorSummator="Error" wizardCaption="Add/Edit P APPLICATION " wizardFormMethod="post" PathID="P_APPLForm" returnPage="p_application.ccp" customInsertType="SQL" customUpdateType="SQL" customDeleteType="SQL" parameterTypeListName="ParameterTypeList" activeCollection="ISQLParameters" customDelete="DELETE FROM p_application WHERE p_application_id = {P_APPLICATION_ID}" pasteActions="pasteActions" wizardTheme="sikm" wizardThemeVersion="3.0" pasteAsReplace="pasteAsReplace" dataSource="SELECT * 
FROM p_application
WHERE p_application_id = {P_APPLICATION_ID} " customInsert="INSERT INTO p_application(p_application_id,code,description,creation_date,created_by,updated_date,updated_by, listing_no, is_active) VALUES 
(generate_id('ifl','p_application','p_application_id'),trim(upper('{CODE}')),trim('{DESCRIPTION}'),current_date,'{CREATED_BY}',current_date,'{UPDATED_BY}', {LISTING_NO}, '{IS_ACTIVE}')" activeTableType="customUpdate" customUpdate="UPDATE p_application SET 
...
WHERE p_application_id = {P_APPLICATION_ID}">
			...
			<SPParameters/>
			<SQLParameters>
				<SQLParameter id="211" parameterType="URL" variable="P_APPLICATION_ID" dataType="Float" parameterSource="P_APPLICATION_ID"/>
			</SQLParameters>
			<JoinTables>
			</JoinTables>
			<JoinLinks/>
